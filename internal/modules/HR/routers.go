...
func (S *HRService) registerRoutes() {
	...
	// employee routes
	hrRoute.POST("/employee", S.createEmployee, rba.AuthMiddelware([]string{"admin", "mod"})) // this
	...
	hrRoute.POST("/employee/create-emp-link", S.CreateEmpLink)
	...
	hrRoute.POST("/getpayroll", S.getPayroll, rba.AuthMiddelware([]string{"admin", "mod"}))
	...
	hrAirT.PUT("/update-status/:id", S.UpdateAirTicketRequestStatus, rba.AuthMiddelware([]string{"admin", "mod"}))
}
