...
const getEmployeeAttendanceReport = `-- name: GetEmployeeAttendanceReport :many
SELECT 
...
type GetEmployeeAttendanceReportParams struct {
	EmpID      int64       `json:"emp_id"`
...
type GetEmployeeAttendanceReportRow struct {
	Date      time.Time `json:"date"`
	EmpID     int64     `json:"emp_id"`
...
func (q *Queries) GetEmployeeAttendanceReport(ctx context.Context, arg GetEmployeeAttendanceReportParams) ([]GetEmployeeAttendanceReportRow, error) {
	rows, err := q.db.QueryContext(ctx, getEmployeeAttendanceReport,
		arg.EmpID,
		arg.Column2,
		arg.CreateDate,
		arg.Limit,
	...
	defer rows.Close()
	var items []GetEmployeeAttendanceReportRow
	for rows.Next() {
		var i GetEmployeeAttendanceReportRow
		if err := rows.Scan(
			&i.Date,
			&i.EmpID,
			&i.InTime,
...
const getEmployeeAttendanceReportForAll = `-- name: GetEmployeeAttendanceReportForAll :many
SELECT 
  DATE(a.create_date) as date,
  a.emp_id,
  TIME_FORMAT(MIN(CASE WHEN a.attendance_type = 'in' THEN TIME(a.create_date) END), '%H:%i:%s') as in_time,
...
type GetEmployeeAttendanceReportForAllParams struct {
	Column1    interface{} `json:"column_1"`
	CreateDate time.Time   `json:"create_date"`
	Limit      int32       `json:"limit"`
	Offset     int32       `json:"offset"`
...
type GetEmployeeAttendanceReportForAllRow struct {
	Date      time.Time `json:"date"`
	EmpID     int64     `json:"emp_id"`
	InTime    string    `json:"in_time"`
	OutTime   string    `json:"out_time"`
...
func (q *Queries) GetEmployeeAttendanceReportForAll(ctx context.Context, arg GetEmployeeAttendanceReportForAllParams) ([]GetEmployeeAttendanceReportForAllRow, error) {
	rows, err := q.db.QueryContext(ctx, getEmployeeAttendanceReportForAll,
		arg.Column1,
		arg.CreateDate,
		arg.Limit,
		arg.Offset,
	...
	defer rows.Close()
	var items []GetEmployeeAttendanceReportForAllRow
	for rows.Next() {
		var i GetEmployeeAttendanceReportForAllRow
		if err := rows.Scan(
			&i.Date,
			&i.EmpID,
			&i.InTime,
